    ...
    <div class="flex flex-col h-full">
        ...
        <div class="flex-1 p-4 overflow-y-auto bg-gray-100 dark:bg-gray-800">
            @foreach($record->messages as $message)
                <div class="mb-4 flex {{ $message->sender_id === auth()->id() ? 'justify-end' : 'justify-start' }}">
                    <div class="max-w-xs p-2 rounded-lg {{ $message->sender_id === auth()->id() ? 'bg-green-500 text-black' : 'bg-blue-500 dark:bg-gray-700 text-gray-800' }}">
                        <p>{{ $message->content }}</p>
        ...
        <div class="p-4 bg-white dark:bg-gray-700">
            <form wire:submit.prevent="sendMessage" class="flex">
                ...
                <button type="submit" class="px-4 py-2 bg-green-500 text-black rounded-r-lg hover:bg-green-600">Send</button>
            </form>
